...
latencies = [1:1:5];   % Indexes and values are going to be the same
Ur = zeros(nMDP,nS,nA);
Ur(1,1,1) = 1;
reinforcement_schedule = zeros(nMDP,nS,nA,nL);  % Define probability of reward for each (state, action, latency) triplet
reinforcement_schedule(1,1,1,:) = 1;
Cu = zeros(nA);
Cv = zeros(nA);
Cu(1) = 0.25; Cu(2) = 0; 
Cv(1) = 0.25; Cv(2) = 0;
